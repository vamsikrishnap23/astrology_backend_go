...
import (
	...
	"github.com/vamsikrishnap23/astrology_backend_go/internal/astrology/kp"
...
func FourStepSignificatorsHandler(w http.ResponseWriter, r *http.Request) {
	...
	sigRes := kp.CalculateSignificators(tblRes)
	fourStepRes := kp.CalculateFourStepSignificators(tblRes, sigRes)

	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(fourStepRes)
}
