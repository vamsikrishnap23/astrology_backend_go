...
func main() {
	...
	// Geocentric
	endBoundary := 336.0
	endJD_Geo := bisectionSearch(jd, jd+2.5, endBoundary, calcTithiAngleGeocentric)
	
	// Topocentric
	calcTopo := func(j float64) float64 { return calcTithiAngleTopocentric(j, lat, lon) }
	endJD_Topo := bisectionSearch(jd, jd+2.5, endBoundary, calcTopo)

	fmt.Printf("Geocentric End Time: %s\n", jdToUTC(endJD_Geo).In(loc).Format(time.RFC3339))
	fmt.Printf("Topocentric End Time: %s\n", jdToUTC(endJD_Topo).In(loc).Format(time.RFC3339))
}
