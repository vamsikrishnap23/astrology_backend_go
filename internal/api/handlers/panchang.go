...
// DailyPanchangHandler handles daily panchang timeline calculations.
func DailyPanchangHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	res, err := panchang.CalculateDailyPanchang(ctx)
	if err != nil {
		http.Error(w, "Error calculating daily panchang", http.StatusInternalServerError)
		return
	}
